...
const onSubmit = async (data) => {
  try {
    const result = await login(data).unwrap(); // result contains your API response
    console.log('Login success:', result.user);

    // Navigate based on role
    if (result) {
      navigate('/admin/management/citizens/', { replace: true });
    } else {
  ...
  if (user) {
    return <Navigate to="/admin/management/citizens/" replace />;
  }
