...
import AdminUserGridSkeleton from "../../components/skeltons/AdminUserGridSkelton";
...
const SolverManagement = () => {
  ...
  const handleCreate = async (createdUser) => {
    try {
      ...
      successToast({
        title: "Solver Created",
        description: "The solver has been created successfully.",
      });
    } catch (err) {
  ...
  return (
    <>
      ...
      {/* User cards */}
      {isFetching || addStatus.isLoading || updateStatus.isLoading  ? (
        <AdminUserGridSkeleton />
      ) : (
